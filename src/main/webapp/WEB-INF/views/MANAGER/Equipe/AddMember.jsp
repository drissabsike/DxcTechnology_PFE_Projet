        ...
        <div class="wrapper wrapper--w900">
            <div class="card card-6">
                <div class="card-heading">
                    <h2 class="title">Ajouter un membre</h2>
                </div>
                <div class="card-body">
                    <form action="/MANAGER/Equipe/process_Add_Member" method="POST">
                      <div class="form-row">
                      <div class="value" style="color:green;position: relative;">${success}</div><br>
                            <div class="value">Nom d'equipe</div>
                            <div class="value">
                                <input class="input--style-6" type="text" name="nom_equipe" value="${nom_equipe}" readonly>
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="value">Employe</div>
                            <div class="value">
								<select class="input--style-6" name="employe" required="required">
									...
									<% try{
										...
										String req="SELECT CONCAT (nom,' ',prenom) as employe FROM `users` WHERE role_user='EMPLOYE'";
										PreparedStatement ps = con.prepareStatement(req);
										ResultSet rs = ps.executeQuery();
										while(rs.next()){
											%>
											<option value="<%=rs.getString("employe")%>"><%=rs.getString("employe")%></option>
											<% 
										}
